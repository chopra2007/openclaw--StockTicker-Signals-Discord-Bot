...
| 34 | Wire in Apify | 1 — not built | $4.65 of the free $5 left. The recommended source (Seeking Alpha) is **dead today** (its server errors out) — exactly the "code works, API doesn't" trap. Re-check after tomorrow's reset; don't build on it yet. |
...
- **Do NOT build now.** The only budget-viable actor (`doesaiknow` Seeking Alpha) returned 0 items / 500 errors today — it's a proxy to a third-party backend outside our control. $4.65 of $5 free; the alternatives cost ~$0.95/run (blow the budget for polling).
...
- **[review revision] The bigger question: is #34 worth building at all?** The bot **already has free per-ticker news** via the `google_rss` cascade tier (config:98) + Finnhub. The ONLY signal Apify-Seeking-Alpha uniquely adds is the **analyst/engagement layer** (`comments_count`) — and its only budget-viable actor is dead today. Gemini suggested Yahoo Finance per-ticker RSS as a free alternative, but I **smoke-tested it and it 404s** (Yahoo deprecated that endpoint) — so that's not a path. Recommendation: treat #34 as **low-priority / likely-skip** unless the `doesaiknow` actor revives with the unique engagement data; the news-coverage gap it was meant to fill is already covered for free.
