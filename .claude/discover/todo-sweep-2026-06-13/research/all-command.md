...
- **Data source / fetch:** `yf.Ticker(t).eps_revisions` — a DataFrame indexed by period (`0q,+1q,0y,+1y`) with columns `upLast30days`, `downLast30days`, `upLast7days`, `downLast7Days`. Use the `0q` (current quarter) row.
- **Where the fetch lives:** `consensus_engine/scanners/snapshot.py` → `_fetch_info()` already calls `t.earnings_estimate` in the SAME blocking call. Add `t.eps_revisions` right beside it (one more attribute read on the already-constructed `yf.Ticker`), stash under a synthetic key like `info["_eps_rev"] = {"up": int(up30), "down": int(down30)}`. **No new fetch, no new latency.**
