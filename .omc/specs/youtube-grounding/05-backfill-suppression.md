# Spec 05 — Backfill Suppression (Layer 5, one-shot DB cleanup)

**Goal:** Mark all already-persisted YouTube rows that fail the new grounding rules as `suppressed=1` with `suppression_reason='hallucination_backfill'`. The hallucinated NVDA rows for `vkqchQQnm88` are the canonical first-batch target.

**Sizing:** SMALL (~130 LOC).

**Important:** Suppress, don't delete. Audit value persists; rows can be flipped back manually if a false positive is identified.

---

## (a) New script: `scripts/backfill_youtube_grounding.py`
...
## (b) Verify against incident video first

Execution order after PR ships:

```bash
# 1. Sanity-check the dry run on the known-bad video
python3 scripts/backfill_youtube_grounding.py --dry-run --video vkqchQQnm88
# Expected log:
#   backfill: video=vkqchQQnm88 title='AMC GAMESTOP KOSS - IT HAS BEGUN!!!...' \
#     candidates=['AMC', 'GME', 'NVDA'] off_allowlist=['NVDA']
#   backfill: ... signals=1 levels=4 setups=1

# 2. Apply on the one video
python3 scripts/backfill_youtube_grounding.py --video vkqchQQnm88

# 3. Verify
sqlite3 consensus.db \
  "SELECT ticker, level_type, price, suppressed, suppression_reason
   FROM youtube_levels WHERE video_id='vkqchQQnm88';"
# Expected: 4 NVDA rows with suppressed=1 reason='hallucination_backfill';
#           AMC + GME rows unchanged.

# 4. Apply globally (only if step 2 looked correct)
python3 scripts/backfill_youtube_grounding.py --dry-run
python3 scripts/backfill_youtube_grounding.py
```
...
`_suppress_off_allowlist` filters `WHERE COALESCE(suppressed, 0) = 0` — already-suppressed rows are not re-touched. Re-running the script after a fresh batch of hallucinations only updates the new ones.
...
```bash
python3 -m pytest tests/scripts/test_backfill_youtube_grounding.py -v
```

Test cases (Spec 06 covers full):
- Fixture DB with vkqchQQnm88 + 3 NVDA setups, AMC/GME spans → script flags NVDA only.
- `--dry-run` makes no DB writes.
- Idempotent: running twice produces same final state.
- Rollback SQL works (test as plain `UPDATE` in a fixture).
...
```sql
-- If a false-positive suppression is found, reverse it for one row:
UPDATE youtube_levels
SET suppressed = 0, suppression_reason = NULL
WHERE id = <row_id>;

-- Or roll back the entire backfill in one shot:
UPDATE youtube_signals  SET suppressed = 0, suppression_reason = NULL
  WHERE suppression_reason = 'hallucination_backfill';
UPDATE youtube_levels   SET suppressed = 0, suppression_reason = NULL
  WHERE suppression_reason = 'hallucination_backfill';
UPDATE youtube_setups   SET suppressed = 0, suppression_reason = NULL
  WHERE suppression_reason = 'hallucination_backfill';
UPDATE youtube_options  SET suppressed = 0, suppression_reason = NULL
  WHERE suppression_reason = 'hallucination_backfill';
```
...
- Backfilling `youtube_macro` rows — macro thesis isn't ticker-specific in our data model.
- Cleanup of `youtube_evidence_spans` themselves — Layer 2 only filters at write time; existing spans with hallucinated `tickers_json` arrays are not rewritten (audit value preserved; new write path is grounded).
