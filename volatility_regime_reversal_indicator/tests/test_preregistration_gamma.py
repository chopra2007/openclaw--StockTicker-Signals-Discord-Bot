"""The frozen Phase-GAMMA pre-registration must parse and carry its required sections + the
honesty disclosures (post-selection flag, SQZ-coverage limitation, opportunity-set null, OOS
battery incl. the QQQ transfer), the 7-condition kill-gate, the alert budget, and the benchmark
battle."""
...
_PRE = Path(__file__).resolve().parent.parent / "backtest" / "preregistration_gamma.yaml"
...
def test_required_sections_present() -> None:
    ...
    for k in ["meta", "primary", "target_events", "min_effect_size", "constructions",
              ...
              "kill_gate"]:
        assert k in d, f"missing Phase-GAMMA pre-registration section: {k}"
...
def test_honesty_disclosures_frozen() -> None:
    ...
    assert d["meta"]["phase"] == 5
    assert d["meta"]["name"] == "gamma"
    assert d["meta"]["builds_on"] == "phase3_NO_GO"
    assert d["meta"]["backtest_start"] == "2011-05-02"
    assert "sqz_coverage_note" in d["meta"]
