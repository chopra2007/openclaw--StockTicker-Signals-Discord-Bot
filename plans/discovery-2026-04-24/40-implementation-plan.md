# Phase 4 Implementation Plan — Discovery 2026-04-24

**Date:** 2026-04-24
**Branch:** `claude/multi-agent-tmux-setup-zWYEQ`
**Scope:** Implementation plan for the 9 surviving features (F1, F2, F3, F4, F5, F6, F8, F10, F11) and 9 cross-cutting safeguards (S1–S9) converged in `33-final-feature-set.md`.

**Source documents (read first if reviewing):**
- `33-final-feature-set.md` — source of truth: hardened feature descriptions, kill criteria, instant-trigger eligibility, cross-cutting safeguards
- `00-system-map.md` — capability map, pipeline `file:line` references, in-code constraints
- `20-candidate-features.md` — original 14 candidates with composite scoring
- `30-critique-signal-quality.md` / `31-critique-feasibility.md` / `32-critique-adversarial.md` — regime / feasibility / adversarial critiques
- `plans/AUDIT_RESEARCH_2026-04-24.md` — pre-existing bug audit (M3 cooldown race, Phase-2 78.4% silent-drop, dead `regime_detector.py`, etc.)

**Spec scaffold:** `docs/superpowers/specs/discovery-2026-04-24-features.md` (canonical feature IDs / names / hardened descriptions / kill criteria / module paths)

**Total scope:** ~2,910 LOC across 18 items (9 features + 9 safeguards).
**Milestone-1 preconditions:** S1 (shared SEC EDGAR semaphore), S3 (M3 cooldown generalization), S7 (schema migrations) — must ship before any feature.

## Table of Contents

### PART A — Build Manifest (mechanics)
1. System Overview
2. Component Architecture (9 features + 9 safeguards)
3. Data Flow Pipeline
4. Data Structures
5. Integration Plan

### PART B — Operations Playbook
6. Failure Handling
7. Test Plan
8. Rollout — feature flags, dark-launch, success metrics, kill switches, milestone sequencing
9. Risks & Open Questions
10. Spec Scaffold

This document was authored by two parallel architects (`disc-p4a-architect` covering §1–5, `disc-p4b-architect` covering §6–10) with a shared canonical module-path manifest, then concatenated. Original split files (`40a-implementation-mechanics.md`, `40b-implementation-operations.md`) preserved in git history.
