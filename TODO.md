...
**CURRENT STATUS (2026-07-03):** All 15 fixes LIVE; 2 safe switches ON (C4 `llm.score_fallback_enabled`, C12 `options_flow.staleness_failclosed`); 5 cautious ones deliberately OFF for the 7-day soak. **Nothing to do until the soak timer fires Mon 2026-07-05 09:00 PDT** (`task_1782704382_12c893.timer`, Persistent) — it writes a flip/hold recommendation to `notifications.log`; it does NOT auto-flip. Then flip the safe ones one at a time per the day-7 rules in the detail file (C3+C14 internal-only immediately; C19 if week's validation-error count ≈ 0; C2+C5 together only if backoff data shows genuinely-dead sources), restarting + confirming clean between each. Bonus already observed: the circuit breaker's shadow mode is correctly marking `exa` OPEN (HTTP 402) all day while `enabled=false` guarantees it never blocks a real request.
...
**CURRENT STATUS (2026-07-03):** Big progress. **Part 1 SHIPPED** (detection + safety net, no dependency): (1) the every-5-min `ci-monitor.sh` now extracts the REAL failing test ids from the FULL CI log — proven on the 07-02 pyarrow run it correctly names `tests/test_market_command.py::…`, where the old `--log-failed` returned nothing; (2) `session_close.sh` now captures the push exit code and writes a loud `notifications.log` line when a push is rejected (the silent hole that stranded 6 commits); (3) a SessionStart banner (`openclaw-digest.sh`) surfaces any GATE/CI/PUSH alert loudly; (4) 07-02 root cause fixed — `scripts/pre-push` uses a per-user `/tmp` log (a stale root-owned one permission-denied `tee` → aborted the hook → silent reject), synced to `.git/hooks/pre-push`, stale files cleared; `notifications.log` made openclaw-writable. **Part 2 STAGED but DORMANT** — `/root/task_system/scripts/ci_autofix.sh` (the real auto-check-fix-repush: capped retries, local-repro to reject flaky, a HARD forbidden-path gate that never auto-pushes a flag/vision/go-live/CI change, local re-verify before repush, loud escalation). It CANNOT fire until claude is installed+authed for the NON-root `openclaw` user — a credential decision surfaced to the user 2026-07-03. Until then ci-monitor safely falls through to loud notify-only. (pyarrow live symptom already fixed, `ed143c9`.)
