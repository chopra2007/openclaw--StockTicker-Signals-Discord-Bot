...
**CURRENT STATUS (2026-07-10) — REOPENED.** The detection + deterministic layers are done and stand (Part 1 + missing-package auto-fix). What's reopened is the **AI logic-bug fixer's model choice.** The 07-09 race that pinned `qwen/qwen3-coder-next` at 1-in-5 tested a **weak field** — mostly flash/mini/small models (gpt-oss-120b, glm-4.7-flash, qwen3-coder-30b, deepseek-v4-flash, gemini-2.5-flash) plus one real coding model. So the conclusion "only 1-in-5 is achievable" is an artifact of bad model selection, not a real ceiling. Redo the race with a **strong-coding field** (cost is a non-issue — the user confirmed 10c/month is fine; every strong model is under 40c/month). **Do NOT re-pin qwen.** Next-steps + full findings in the detail file. Two on-disk fixes from this session are unstaged and worth keeping (streaming fallback + fake-green guard — see detail file). 
