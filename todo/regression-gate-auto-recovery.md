...
**Status:** IN PROGRESS (2026-07-03) — Part 1 (detection + safety net) SHIPPED; Part 2 (auto-fixer) STAGED but DORMANT.
  ...
  - **Part 2 STAGED:** `/root/task_system/scripts/ci_autofix.sh` written (capped retries, local-repro flaky-reject, HARD forbidden-path gate = never auto-push a flag/vision/go-live/CI change, local re-verify, loud escalation). CANNOT run until claude is installed+authed for the NON-root `openclaw` user — a credential-provisioning decision surfaced to the user 2026-07-03. ci-monitor stays notify-only until then.
**Created:** 2026-07-01
