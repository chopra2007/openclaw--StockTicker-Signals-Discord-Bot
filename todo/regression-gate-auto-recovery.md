...
**CURRENT STATUS (2026-09-04) — ACTIVE; built, but the required successful live proof has not happened.** Real CI failures now reach `/root/task_system/logs/ci-autofix.log`: five August 24–26 runs reproduced a missing-`databento` failure and escalated after neither model produced a working patch, and two August 28 runs were correctly classified as passing locally twice. None completed the required detect, fix, verify, and re-push chain. `ci-monitor.timer` is active and checks every five minutes. Keep it running and close this item only after a genuine red CI run completes the full chain successfully; do not create a fake failure.

**Status (historical):** ACTIVE — the v3 test plan at `.omc/plans/ci-fixer-race-v3-2026-07-10.md` (now EXECUTED). It supersedes the ROUND 2 section below (do NOT run round 2 as-is — it inherits the flaws). A 2026-07-10 late-session audit found the round-1/retest races were graded on a flawed exam, so their verdict ("no ≤25¢ model works; kimi 38¢ is the only passer") is unproven: (1) the single benchmark bug is a TEST-file bug (human fix `db47044` edits only `tests/test_wolf_digest.py`) scored as if it were a source bug — deepseek-v4-pro's "fake green" was actually the human's exact fix, and production's own fake-green guard would escalate the CORRECT fix for this case, making it unwinnable by design; (2) the ≥70% bar was applied to single tries, but production `ci_autofix.sh` retries 3× (`AI_ATTEMPTS=3`) — per-incident success is 1-(1-p)³, so the "1/3 failures" (qwen3-max, glm-5) sat AT the bar, not below it; (3) 10 of 30 trials died to harness artifacts (glm-5's whole 0/3 was the 300s deadline — it finished 3/3 times at 600s; plus JSON/search-string rejections with no repair round, no response_format enforcement, uncapped reasoning tokens); (4) field gaps (mimo-v2.5 #2, minimax-m3 #3, step-3.7-flash #9 on the programming leaderboard — never raced) and one impossible entrant (coder-large: 32k context < the ~45k-token prompt). The v3 plan: mine git history for 4 genuine source-bug cases (stage via `git checkout <sha>^ -- <src files>` so the reference fix is source-only by construction), harden the harness (JSON enforcement, one repair round, 600s deadline, reasoning caps, provider logging) and TRIM THE PROMPT (~45k→~20k tokens — halves every attempt's cost, race and production alike), then race the under-25¢ field in price bands cheapest-first (screen 1 trial/case, deep 5 trials/case on advancers, per-incident scoring), STOPPING at the first qualifier so pricier models never run; confirm at production settings, then pin. Test spend ≤$3 HARD (user 2026-07-10: the race must undercut ~$4-6/yr for a frontier model; expect $1-2, re-runnable at the same price when models churn). Raw so far: `.omc/ci_fixer_trials_raw.json` + `.omc/ci_fixer_trials_timeout_retest.json`.
...
- **Worked on:** Executed the v3 CI-fixer race plan end to end. Phase 0: mined git history, validated a 4-case source-bug corpus (each red-when-source-reverted / green-at-HEAD, deterministic). Phase 1: hardened `ci_ai_fixer.py` (JSON enforcement, one repair round, 600s deadline, 16k max_tokens, prompt trim to ~20k, context guard, per-attempt logging) + rewrote `ci_fixer_trials.py` for per-incident scoring + banded cheapest-first; 38 unit tests pass; zero-cost mock proved a source fix scores pass and a test-only fix is rejected. Phases 2-4: anchor probe (kimi 4/4 validates the exam), Band A screen, deep-tested the cheapest advancer first.
- **Decisions:** Dropped the mined `e2_multiplier_inversion` case as unfair — the failing test never surfaced `engine.py` to the model (not imported, not in the traceback), so it tested the harness's file-finding, not model capability. Swapped in `health_allowed_mentions`. Prompt trim was the real lever: it put the whole field under 25¢/mo, making price a non-constraint. Pinned `deepseek/deepseek-v4-flash` — cheapest AND capable, so it wins by construction; no pricier band was ever run.
- **Next:** Soak until 2026-07-25 — watch for a real logic-bug red gate the fixer handles live, confirm the @-mention + fix-and-hold path, then mark #59 DONE. Re-race (same harness/corpus, ~$1-2) whenever the pinned slug is retired.

### Session notes — 2026-07-11 (follow-up: backup model)
- **Worked on:** Added a backup model to the CI auto-fixer per user request. `/root/task_system/scripts/ci_autofix.sh` refactored into a `try_model` function run primary→backup: deepseek-v4-flash (primary, via DEFAULT_MODEL) gets 3 attempts; if it strikes out, `stepfun/step-3.7-flash` (different vendor, also 4/4 in the race) gets 3 attempts before paging a human.
- **Decisions:** The two SAFETY stops (forbidden-path, fake test-only green) escalate immediately for either model — no fall-through to backup. Only capability misses (misclassify / no working patch) hand off. Verified with a stubbed fixer: primary-fail→backup-fixes, primary-ok→backup-skipped, both-fail→escalate.
- **Next:** Unchanged — soak until 2026-07-25 for one real red gate to exercise the live chain end-to-end, then close #59.

### Session notes — 2026-09-04

- **Worked on:** Refreshed the waiting record from the primary repair log and live timer only. No monitor or fixer code changed.
- **Proof:** Five genuine August 24–26 failures were reproduced and escalated without a working patch. Two August 28 failures passed locally twice and were left untouched. The five-minute `ci-monitor.timer` is active.
- **Next:** Wait for a genuine red CI run that completes detection, repair, verification, and the configured hold-or-re-push path. Do not manufacture one.
