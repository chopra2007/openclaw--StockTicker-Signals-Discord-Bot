...
- **Help-embed polish (later same session, commits f9c4cd1 + fca55f2):** (1) spelled out the placeholders — `<T>` → `<ticker>` (plus `<url>`/`<question>`/`<channel>`/`<video id>`/`<feature>`) — and gave **all 30 commands a one-line description** so the list is self-explanatory; (2) final **accent color = gold `0xF1C40F`** (user picked "Gold" from 3 yellow shades, after comparing red/green/blue then yellow/purple/orange live in #chat — the earlier "slate 0x2F3136" above is superseded). All live-verified by reading the embed back; the live gold `!help` is the example left in #chat.
