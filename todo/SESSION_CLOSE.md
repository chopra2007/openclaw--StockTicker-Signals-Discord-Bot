...
4. **Merge this session's PR — only if the session worked on a branch other than `master`.** (A session working directly in the main checkout on `master` has no PR; skip this step.) From the session's own worktree, run exactly this one line and nothing else:
   `nohup scripts/merge_session_pr.sh <branch> > /dev/null 2>&1 &`
   **Never push the branch or open the PR yourself here — the script does both.** A plain `git push` fires the pre-push hook, which runs the whole test suite (~7-8 minutes); doing that in the session made "bye" block for 12 minutes, which defeats the entire point of "bye". Everything slow belongs inside the background script.
