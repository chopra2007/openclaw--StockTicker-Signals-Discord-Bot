...
### ⏳ AWAITING USER DECISION (2026-06-24) — E2 master flip
**Decision needed before flipping `features.cross_asset.enabled` ON.** Two things to weigh:
1. **Direction:** as built, E2 makes a STRONG alert *easier* to fire during market stress (backwardation → multiplier 0.85 → lowers the bar) and *harder* during calm (contango → 1.15 → raises the bar). Confirm this contrarian direction is intended — or whether it's inverted vs the "veto bullish signals in stress" framing the feature was described with.
2. **Blast radius:** the offline replay over 2,894 past alerts shows combined E2 would change **~36% of STRONG classifications** (215/600 demoted on calm days). That's a large live-alert change for a multiplier that has no shadow-first apply mode.
**Status:** shadow mode is ON now (logs only, no live effect) and is accruing the real distribution to inform this. Master stays OFF until the user rules on direction + accepts the impact. Evidence: `.claude/discover/todo-sweep-2026-06-24/e2-replay-result.md`.
