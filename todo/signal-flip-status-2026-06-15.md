...
- **E6 `manufactured_agreement_gate` → SHADOW (enabled:false, shadow:true).** Built a `shadow` mode (`cross_reference.py:1581+`, mirrors E2): computes + logs the burst/corroboration decision with the FULL sec+catalyst+options set at real event time, but does NOT mutate `consensus_boost` and does NOT feed I3 (truly inert; new unit test `test_shadow_mode_does_not_gate_or_feed_i3`). Why shadow not live: event-time backtest (`scripts/backtest_e6_manufactured_burst.py`) found **11 bursts in 4613 twitter signals / 880 tickers** (rare), of which **3/11 (27%) would-gate UPPER BOUND** — but all 3 are **META/NVDA mega-cap legit-consensus** and the backtest CANNOT reconstruct news/options corroboration (only sec is event-time-stored), so true gating is likely <3. 27% > Codex's 20% auto-flip gate → **do not flip live**; shadow soak gathers the missing event-time news/options corroboration. **Live flip deferred** to a later session after the soak, likely with `min_accounts→3` or `similarity→0.70`.
...
- **Unchanged:** E2 master still OFF (the +42% calm-promotion TUNING decision above is the user's call). I7/I14-widening still OFF (proven no-ops). I4-full/I3/I10/I15/E1/FRED-leg still ON.
