...
Detail: **`todo/options-flow-realtime.md`** (= TODO #18). **HARD CONSTRAINT: FREE only — paid feeds are OFF the table (user).** Research the freshest FREE data source FIRST (yfinance delayed chains / Firecrawl-scrape of a free unusual-activity page / free API tiers — confirm Polygon/Tradier free tiers actually return options before relying on them). Reuse the DORMANT `scan_unusual_options_market` + the existing instant-trigger alert policy, define "unusual" thresholds, then build + verify. Realistic free ceiling ≈ 15-min-delayed; "close to real-time, not 24h" means ~15-min-fresh.
