...
**REMAINING:** A1/A2/A3 check chart-derived levels/setups/catalysts (`youtube_levels`/`youtube_setups`/`youtube_catalysts`). A5/A4/A6/A7 pass.

**CORRECTION — 2026-05-29 (later):** an earlier note here said "that wiring IS #17 Task C." **That was wrong.** #17 Task C (Phase 1+2, shipped) wires `youtube_visual_evidence` → the **`!all` narrator prompt**, NOT into the classifier's `youtube_levels`/`setups`/`catalysts` tables that A1-A3 read. So Task C does **NOT** fix A1-A3. To make A1-A3 pass, the **classifier** has to produce those levels for the eval video `4mSyMr8PGLI` — i.e. either the levels are spoken (classifier extracts them from spans; the 2026-05-29 forced-Gemini run produced `levels=0`, so they may be chart-only or extraction missed them) OR a NEW `visual_evidence → classifier → youtube_levels` wiring is built (not Task C, not yet planned). **Net: #3 root cause is fixed (cron tests Gemini, 4/7); A1-A3 are decoupled from Task C and remain open.**
