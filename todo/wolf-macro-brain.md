...
- **Phase-4 follow-ups (tracked):** large RS deltas verified REAL (MU +96% in 21d on the melt-up tape) → **anti-chase guard added (commit 98e0de9, LIVE):** names up ≥45% over the window are dampened, capped 🟡, flagged "⚠ extended — chase risk" (SNOW/DDOG flagged; GS/CVX stay 🟢). Remaining: confirm macro_universe asset-direction convention (bull=up) vs newsletter wording; rs_window stays 1-month (user choice; 63d was research rec); shorts side deferred to v1.1; widen confluence/flow inputs later. (98e0de9 unpushed — atop concurrent !all commits — pushes at next close.)
