...
- **PERMANENT FIX (user-console, cannot do myself — no gcloud here):** publish the OAuth consent screen for project `akashbot-495306` from **Testing → Production**, and do NOT submit for verification (accept the "unverified app" warning). Researched + confirmed against Google's official docs: the 7-day refresh-token expiry applies ONLY to Testing+External; Production-unverified persists indefinitely for a single owner user (the 100-new-user cap is irrelevant at 1 user). Internal user-type and service-account domain-wide delegation are both unavailable for personal @gmail. Exact steps saved at `.claude/discover/todo-sweep-2026-06-24/gmail-production-fix.md`. Until then, the watchdog is the standing safety net (loud alert before the feed goes dark instead of silent darkness).
