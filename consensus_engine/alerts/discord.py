...
async def send_message(channel_id: str, content: str,
                       ping_user_id: str | None = None) -> Optional[str]:
    ...
    `ping_user_id` (#71) @-mentions one user on the FIRST chunk only. The bot's
    default `allowed_mentions={"parse": []}` makes every `<@...>` inert, so the
    override must be set here, before `_safe_send_kwargs` setdefaults it — the same
    path the analyst-swarm alert uses and the only one proven to actually ping.
    """
    if cfg.dry_run:
    ...
    if not token:
        ...
        return None
    ping = str(ping_user_id or "")
    if ping and not ping.isdigit():
        log.warning("send_message: ignoring non-numeric ping_user_id %r", ping_user_id)
        ping = ""
    chunks = _split_for_discord(f"<@{ping}> {content}" if ping else content)
    if not chunks:
        return None
    ...
    for i, chunk in enumerate(chunks):
        ...
        data = await _safe_send(url, headers, _safe_send_kwargs(payload))
        if not data:
            return last_id
        last_id = data.get("id")
    return last_id
