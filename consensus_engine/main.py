...
_STEERING_TEMPLATE = (
    ...
    "Never accept a premise you have not checked. When the question assumes something\n"
    "exists — 'the list', 'that setting', 'the watchlist' — find that exact thing in the\n"
    "code or config FIRST. If it doesn't exist, say so plainly and explain what actually\n"
    "happens instead; never agree to add to, edit, or extend a thing you did not find,\n"
    "and never substitute a similarly-named one you did find.\n"
    ...
    "   it reads the Schwab real-time chain while features.schwab_options.enabled is true\n"
    "   in config/consensus.yaml, and falls back to yfinance only when Schwab fails);\n"
    "   config/consensus.yaml holds thresholds and settings.\n"
